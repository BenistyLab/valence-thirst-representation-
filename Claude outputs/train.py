...
Design notes (adapted from the paper's Section 4.5):
  ...
  - Loss: CrossEntropyLoss (averaged over all phases in the batch).
    Optionally weighted by inverse class frequency to counter imbalance.
...
def _run_epoch(
    ...
    The criterion is created once in train() and passed in here so that
    class weights (if any) are applied consistently without rebuilding the
    loss object on every epoch.
    ...
    for phase, loader in loaders.items():
        for batch in loader:
            ...
            lengths = batch["lengths"]             # (B,) actual trial lengths
...
def train(
    ...
    *,
    ...
    class_weights:  (N_CLASSES,) float32 tensor of per-class loss weights.
                    Pass the output of compute_class_weights() to counter
                    label imbalance.  None means unweighted loss.
    ...
    # Build criterion once; move class weights to device if provided.
