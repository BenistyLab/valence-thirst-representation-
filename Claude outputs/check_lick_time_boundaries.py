...
# --- debugging -----------------------------------------------------------
# Set this to e.g. "20Hz/AL45" to skip the subprocess machinery entirely and
# run just that one mouse/protocol's computation *in this same process*.
# Breakpoints you set inside _compute_one / _water_trial_phases / etc. will
# then actually be hit by PyCharm's debugger (a subprocess launched with
# subprocess.run is a separate interpreter -- PyCharm Community can't attach
# a debugger to it, so breakpoints inside code that only runs in the worker
# are silently skipped when you Debug the script normally). Leave as None
# for the real multi-mouse run.
DEBUG_MOUSE_ID: str | None = None
...
def _compute_one(npz_path: str, mouse_id: str) -> dict | None:
    ...
    if not valid_trials:
        print(f"{mouse_id}: no water trials with >= 500 frames, skipping.")
        return None

    # Keep only valid trials and first 500 frames of each
    water_500 = pd.concat(
        [water.loc[:, (trial, slice(None))].iloc[:, :500] for trial in valid_trials],
        axis=1,
    )

    # Average across the first 500 frames of each trial
    avg_lick_rate = water_500.groupby(level="trial", axis=1).mean()
...
def main() -> None:
    ...
    if DEBUG_MOUSE_ID is not None:
        # In-process debug path: no subprocess, so PyCharm breakpoints work.
        protocol, mouse = DEBUG_MOUSE_ID.split("/")
        file = by_mouse.get(mouse, {}).get(protocol)
        if file is None:
            raise ValueError(f"No train_pool.npz found for {DEBUG_MOUSE_ID!r} under {PATH}")

        os.environ["LIVNEH_MOUSE_ID"] = DEBUG_MOUSE_ID  # normally set by the subprocess env
        data = _compute_one(str(file), DEBUG_MOUSE_ID)  # <-- set your breakpoint here
        if data is None:
            print(f"{DEBUG_MOUSE_ID}: nothing to plot.")
            return
