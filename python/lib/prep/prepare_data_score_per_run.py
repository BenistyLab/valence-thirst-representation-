...
def _build_pack(
    ...
    for i, L in picked:
        cs = int(cue_start[i])
        raw = dfF[:, cs : cs + L]
        # get relevant mu and sig
        current_run_number = trial_run[i]
        current_phase = run_to_phase_dict[current_run_number]
...
def run_to_phase(phase_runs):
    run_to_phase_dict = {}
    for phase in phase_runs.keys():
        for run_number in phase_runs[phase]:
            run_to_phase_dict[run_number] = phase
    return run_to_phase_dict
...
def main(session_dir: Path | None = None) -> None:
    ...
    train_sel = []
    train_phases = {'pre': [], 'water': [], 'airpuff': []}
    mu_phases = {}
    sig_phases = {}
    ...
    for phase in train_phases:
        mu, sig = _fit_mu_sigma(dfF, train_phases[phase], cue_start)
        mu_phases[phase] = mu
        sig_phases[phase] = sig
